...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
    val partialFunction: PartialFunction[String, String] = {
      case str if isASCIIString(str) => str.toUpperCase
      case str if !isASCIIString(str) => str.toLowerCase
    }
    text match {
      case ::(head, tail) => head :: tail.collect(partialFunction)
      case _ => Nil
    }
  }
  ...
  def numbersToNumericString(text: String): String = {
    ...
    text.flatMap(char => if (char.toString.matches("[0-9]")) map(char.toString.toInt) else char.toString)
  }
